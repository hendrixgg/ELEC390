...
def floyd_warshall_router(map: AdjListMap) -> TripRouter:
    """Returns a TripRouter that will fail if a trip cannot be routed on the map."""
    ...
    # At this point, you would also want to check that the graph is connected, but we are assuming that the graph is connected.
    # If the graph had negative cycles, then the shortest path would not be well-defined, but we are assuming that the graph does not have negative cycles.
...
if __name__ == "__main__":
    ...
    # valid trip plan
    weak_router = floyd_warshall_router(weak_map)
    ...
    strong_router = floyd_warshall_router(strong_map)
    try:
        print(TripPlan.from_trip_request(strong_map, TripRequest(
